...
mod byte_slice {
    ...
    fn doesnt_understand_unicode_boundaries() {
        ...
        assert_eq!(haystack.current_item(), Some("😀".as_bytes()[0]));
        assert_eq!(haystack.current_index(), 0);
        haystack.next();

        assert_eq!(haystack.current_item(), Some("😀".as_bytes()[1]));
        assert_eq!(haystack.current_index(), 1);
        haystack.next();
        haystack.next();
        haystack.next();
        ...
        assert_eq!(haystack.current_item(), Some("🧑".as_bytes()[3]));
        assert_eq!(haystack.current_index(), 7);
        haystack.next();

        assert_eq!(haystack.current_item(), Some("\u{200d}".as_bytes()[0]));
